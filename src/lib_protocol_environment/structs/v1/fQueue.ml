...
let rec cons : type a. a -> a t -> a t =
 fun x q ->
  ...
  | Deep (n, One y, middle, tl) ->
      _deep (n + 1) (Two (x, y)) middle tl
...
let rec snoc : type a. a t -> a -> a t =
 fun q x ->
  ...
  | Deep (n, hd, middle, One y) ->
      _deep (n + 1) hd middle (Two (y, x))
...
let rec take_back_exn : 'a. 'a t -> 'a t * 'a =
 fun q ->
  ...
  | Deep (n, hd, middle, Two (x, y)) ->
      (_deep (n - 1) hd middle (One x), y)
...
let rec nth_exn : 'a. int -> 'a t -> 'a =
 fun i q ->
  ...
  | (1, Shallow (Two (_, x))) ->
      x
  | (0, Shallow (Three (x, _, _))) ->
      x
  | (1, Shallow (Three (_, x, _))) ->
      x
  | (2, Shallow (Three (_, _, x))) ->
      x
  | (_, Shallow _) ->
      raise Not_found
  | (_, Deep (_, l, q, r)) ->
